...
Ouline只有TAP模式而且只支持全局IP代理，没有进程模式。

Netch有TAP模式和进程模式，但无法同时使用，TAP模式支持IP分流，进程模式只能全局代理。

Clash For Windows的TAP模式支持域名分流和IP分流，但没有进程模式。

Mellow的TAP模式同时支持域名分流，IP分流，进程模式，且进程模式支持域名分流和IP分流。
