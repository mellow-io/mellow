...
Outline：只有TAP模式且只支持全局IP代理，没有进程模式。

Netch：有TAP模式和进程模式还有系统代理，TAP模式只支持IP分流，进程模式只能全局代理，但TAP模式和进程模式无法同时使用。

Clash For Windows：有TAP模式和系统代理，TAP模式支持域名分流和IP分流，没有进程模式。

Mellow：有TAP模式和进程模式还有系统代理，TAP模式支持域名分流和IP分流，进程模式支持域名分流和IP分流，且TAP模式和进程模式可以同时使用。
